...
import { CreateScheduleGroupCommand, SchedulerClient } from '@aws-sdk/client-scheduler'
...
export interface AwsConfig {
  ...
  /**
   * Auto-start a local LocalStack Docker container (DynamoDB + SQS) and use it
   * as the endpoint, instead of talking to real AWS. Env: WORKFLOW_AWS_LOCAL.
   */
  local?: boolean
...
export function resolveAwsConfig(config: AwsConfig = {}): ResolvedAwsConfig {
  const region = config.region ?? process.env.WORKFLOW_AWS_REGION ?? process.env.AWS_REGION ?? 'us-west-2'

  const endpoint = config.endpoint ?? process.env.WORKFLOW_AWS_ENDPOINT

  const accessKeyId = config.accessKeyId ?? process.env.WORKFLOW_AWS_ACCESS_KEY_ID
  const secretAccessKey = config.secretAccessKey ?? process.env.WORKFLOW_AWS_SECRET_ACCESS_KEY
  ...
  return {
    ...
    endpoint,
    credentials,
    local: config.local ?? envBool(process.env.WORKFLOW_AWS_LOCAL) ?? false,
    tableName: config.tableName ?? process.env.WORKFLOW_DYNAMODB_TABLE_NAME ?? 'workflow',
    autoProvision: config.autoProvision ?? envBool(process.env.WORKFLOW_AWS_AUTO_PROVISION) ?? true,
    queueUrl: config.queueUrl ?? process.env.WORKFLOW_SQS_QUEUE_URL,
    queueName: config.queueName ?? process.env.WORKFLOW_SQS_QUEUE_NAME ?? 'workflow-queue',
    schedulerGroupName: config.schedulerGroupName ?? process.env.WORKFLOW_SCHEDULER_GROUP_NAME ?? 'workflow',
    schedulerRoleArn: config.schedulerRoleArn ?? process.env.WORKFLOW_SCHEDULER_ROLE_ARN,
    queueArn: config.queueArn ?? process.env.WORKFLOW_SQS_QUEUE_ARN,
    appsyncEventsEndpoint: config.appsyncEventsEndpoint ?? process.env.WORKFLOW_APPSYNC_EVENTS_ENDPOINT,
    appsyncApiKey: config.appsyncApiKey ?? process.env.WORKFLOW_APPSYNC_API_KEY,
  }
}
...
async function waitForTableActive(rawDdb: DynamoDBClient, tableName: string, timeoutMs = 60_000): Promise<void> {
  const deadline = Date.now() + timeoutMs
  while (Date.now() < deadline) {
    try {
      const res = await rawDdb.send(new DescribeTableCommand({ TableName: tableName }))
...
export async function ensureTable(clients: AwsClients): Promise<void> {
  ...
  try {
    ...
    if (!(err instanceof Error) || !/already exists|ResourceInUse/i.test(err.name + err.message)) {
      throw err
    }
  }
...
export async function ensureQueue(clients: AwsClients): Promise<string> {
  ...
  try {
    const res = await sqs.send(new GetQueueUrlCommand({ QueueName: config.queueName }))
    if (res.QueueUrl) {
...
export async function ensureSchedulerGroup(clients: AwsClients): Promise<void> {
  ...
  try {
    await scheduler.send(new CreateScheduleGroupCommand({ Name: config.schedulerGroupName }))
